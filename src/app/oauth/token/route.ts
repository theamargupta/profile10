...
export async function POST(request: NextRequest) {
    ...
    return NextResponse.json(
      {
        ...
        error_description: "Expected form-encoded body.",
      },
      { status: 400 },
    );
  }
  ...
  const clientId =
    typeof form.get("client_id") === "string"
  ...
  try {
    ...
    const validated = validateTokenRequestScopeAndResource({
      scope:
        typeof form.get("scope") === "string"
          ...
          : undefined,
      resource:
        typeof form.get("resource") === "string"
    ...
    if (grantType === "authorization_code") {
      const code =
        typeof form.get("code") === "string" ? String(form.get("code")) : null;
      const codeVerifier =
        typeof form.get("code_verifier") === "string"
          ? String(form.get("code_verifier"))
          : null;
      const redirectUri =
        typeof form.get("redirect_uri") === "string"
    ...
    console.error("[amargupta-tech OAuth] token failed", error);
    return NextResponse.json(
      {
        error: "invalid_grant",
        error_description:
