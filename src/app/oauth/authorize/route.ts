...
const DEFAULT_OPERATOR_EMAIL = "theamargupta.tech@gmail.com";

function operatorEmail(): string {
  return (
    process.env.AMARGUPTA_OPERATOR_EMAIL?.trim().toLowerCase() ||
...
async function authorizeOrError(
  ...
  try {
    ...
    return NextResponse.redirect(buildRedirectUri(redirectUri, { code, state }), {
      status: 303,
    });
  } catch (error) {
    console.error("[amargupta-tech OAuth] authorize failed", error);
...
// Single-operator gate: only the configured operator email can mint tokens.
// Belt-and-suspenders against a stray Supabase signup on the shared project.
function gateOperator(email: string | null | undefined) {
...
export async function GET(request: NextRequest) {
  ...
  if (!user) {
    ...
    adminUrl.searchParams.set("next", next);
    return NextResponse.redirect(adminUrl);
  }

  if (!gateOperator(user.email)) {
    return NextResponse.json(
      {
        error: "access_denied",
        error_description:
          "This MCP server is single-operator. Signed-in account is not the operator.",
      },
      { status: 403 },
    );
  }
  ...
  if (!params.clientId || !params.redirectUri || params.responseType !== "code") {
    return NextResponse.json(
      {
        error: "invalid_request",
        error_description: "Missing OAuth authorization parameters.",
      },
      { status: 400 },
    );
  }
  ...
  const resourceText =
    params.resource || `${request.nextUrl.origin}/api/mcp`;
...
<html lang="en">
  ...
  <body>
    <form class="card" method="post">
      ...
      <p>Confirm access so ${escapeHtml(clientName)} can call your amargupta.tech blog tools (read/write).</p>
...
export async function POST(request: NextRequest) {
  ...
  if (!gateOperator(user.email)) {
    return NextResponse.json(
      {
        error: "access_denied",
        error_description: "Signed-in account is not the operator.",
      },
      { status: 403 },
    );
  }
