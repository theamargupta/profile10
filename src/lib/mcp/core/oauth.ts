...
/**
 * OAuth 2.1 + PKCE + RFC7591 Dynamic Client Registration for amargupta.tech's
 * single-operator MCP server. Mirrors setu-nextjs/lib/mcp/core/oauth.ts but:
 *   - drops the multi-tenant ADDITIONAL_VALID_RESOURCES (single host)
 *   - resource binding is enforced — tokens issued by amargupta.tech only
 *     validate against https://amargupta.tech/api/mcp (and dev-server origin)
 *
 * The mcp_oauth_clients / mcp_oauth_authorization_codes / mcp_oauth_tokens
 * tables are SHARED with setu-nextjs in the same Supabase project
 * (avcnoywxnkajfuobftmr). Cross-app token use is blocked at validateResource —
 * each token carries the issuing origin's resource URL.
 */
...
// Per RFC 8252 §7.1 — private-use URI schemes for native apps.
const ALLOWED_NATIVE_REDIRECT_SCHEMES = new Set<string>([
  "cursor:", // Cursor IDE — anysphere.cursor-mcp/oauth/callback
]);

// Per RFC 8252 §7.3 — native apps SHOULD use the loopback IP literal.
...
export function buildProtectedResourceMetadata(
  ...
  return {
    ...
    resource_name: "amargupta.tech — Personal MCP",
    resource_documentation: new URL("/admin", origin).toString(),
  };
}
...
function isAllowedRedirectUri(uri: string): boolean {
  ...
  if (ALLOWED_NATIVE_REDIRECT_SCHEMES.has(parsed.protocol)) return true;
...
export async function registerOAuthClient(input: {
  ...
  const redirectUris = Array.from(
    new Set(input.redirectUris.map((uri) => uri.trim()).filter(Boolean)),
  );
...
export async function getOAuthClient(
  ...
  const issuedAt = data.created_at
...
export async function exchangeAuthorizationCode(input: {
  ...
  const { data, error } = await supabase
    .from("mcp_oauth_authorization_codes")
    .select(
      "id, client_id, user_id, redirect_uri, code_challenge, code_challenge_method, scopes, resource, expires_at, used_at",
    )
...
export async function exchangeRefreshToken(input: {
  ...
  const nextScopes = input.scopes?.length
    ? input.scopes
    : data.scopes || ["mcp:tools"];
  ...
  const nextResource =
    input.resource === undefined ? data.resource || null : input.resource;
...
export async function verifyOAuthAccessToken(token: string): Promise<AuthInfo> {
  ...
  return {
    ...
    extra: { userId: data.user_id, clientId: data.client_id },
  };
}
...
export function validateTokenRequestScopeAndResource(input: {
  ...
  const resource =
    input.resource === undefined
      ? undefined
      : validateResource(input.resource, input.origin);
  return { scopes, resource };
}
